...
from sample_dummy import sample_dummy
...
def main():
    ...
    if rf == 1:
        eval_complex(rf,rf_nm,inputs,known_complex_nodes_list,prot_list,myGraph)
    else:
        start_time_sample = time_time() 
        prof = cProfile_Profile()
        pred_comp_list,num_comp = prof.runcall(sample_dummy,inputs,myGraph,modelfname,scaler,max_size_train,max_size_test,prot_list)
