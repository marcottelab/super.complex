  ...
  #  @profile
   ...
   # @profile
    ...
    def find_imp_neig(self,neig_list,g1,tempG):
        ...
        sorted_ind = np.argsort(neig_wt_list)
        sorted_wts = [{'weight':val} for val in np.sort(neig_wt_list)][-n_maxs:]
        ...
        if len(imp_neigs) == 1:
            ...
            if  cur_trial <= explore_prob: 
                ...
                max_neig = max(iter(scores.items()), key=operator.itemgetter(1))[0]  
                imp_neig = max_neig          
    ...
    def sample(self,num_comp,run_mode,seed_mode):
        ...
        if(seed_mode == "cliques"):
            ...
            if(run_mode == "parallel"):
                pred_comp_list = Parallel(n_jobs=num_cores,prefer="threads")(delayed(unwrap_self_isa)(node) for node in tqdm(list(zip([self]*num_comp,seed_nodes))))
            else:
                pred_comp_list = [self.search_isa(node) for node in tqdm(seed_nodes)]              
                
        elif(search_method == "metropolis"):
            if(run_mode == "parallel"):            
                pred_comp_list = Parallel(n_jobs=num_cores,prefer="threads")(delayed(unwrap_self_metropolis)(node) for node in tqdm(list(zip([self]*num_comp,seed_nodes))))
            else:
                pred_comp_list = [self.search_metropolis(node) for node in tqdm(seed_nodes)]              
        elif(search_method == "search_top_neigs"):
            if(run_mode == "parallel"):            
                pred_comp_list = Parallel(n_jobs=num_cores,prefer="threads")(delayed(unwrap_self_top_neigs)(node) for node in tqdm(list(zip([self]*num_comp,seed_nodes))))
            else:
                pred_comp_list = [self.search_top_neigs(node) for node in tqdm(seed_nodes)]              
        else:
            if(run_mode == "parallel"):            
                pred_comp_list = Parallel(n_jobs=num_cores,prefer="threads")(delayed(unwrap_self_max_neig)(node) for node in tqdm(list(zip([self]*num_comp,seed_nodes))))
            else:
